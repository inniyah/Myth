...
void rpgchar::doattack(equipment *eleft, equipment *eright, equipment *quiver)
{
	...
	if(attack)
	{
		...
		switch(attack->target)
		{
			...
			case T_SELF:
				hit(this, attack, ammo, mult, flags, vec3(0, 0, 1));
			...
			case T_VERT:
			{
				...
				if(attack->target == T_HORIZ)
					perp = vec3(yaw * RAD, (pitch + 90) * RAD);
				else
					perp = vec3((yaw + 90) * RAD, 0);

				vec3 dir = vec3(yaw * RAD, pitch * RAD).rotate(attack->angle / 2.f * RAD, perp);
				vec3 orig = vec3(o).sub(vec3(0, 0, eyeheight / 2));
				vector<rpgent *> hit;
				vector<vec3> hits;
				...
				{
					vec3 ray = vec3(dir).mul(reach).add(orig);
					loopvj(game::curmap->objs)
					{
						...
						if(obj != this && game::intersect(obj, orig, ray, dist) && dist <= 1)
						{
							...
							hits.add(vec3(ray).sub(orig).mul(dist));
						}
					}
				...
				loopv(hit)
					hit[i]->hit(this, attack, ammo, mult, flags, vec3(yaw * RAD, pitch * RAD));

				loopv(hits)
				{
					vec3 pos = vec3(hits[i]).add(orig);
					if(death) death->drawsplash(pos, vec3(0, 0, 0), 5, 1, effect::DEATH, 1);
				}
			...
			case T_CONE:
			{
				...
				vec3 view = vec3(yaw * RAD, pitch *RAD);
				int reach = (attack->range + (ammo ? ammo->range : 0)) * (flags & CHARGE_TRAVEL ? mult : 1) + radius;
				...
				loopv(game::curmap->objs)
				{
					...
					vec3 other = vec3(d->o).sub(o).normalize();
					const vec3 min = vec3(d->o.x - d->radius, d->o.y - d->radius, d->o.z - d->eyeheight);
					const vec3 max = vec3(d->o.x + d->radius, d->o.y + d->radius, d->o.z + d->aboveeye);

					if(view.dot(other) > sin(attack->angle * RAD) && o.dist_to_bb(min, max) <= reach)
					{
						d->hit(this, attack, ammo, mult, flags, other);

						if(death) death->drawsplash(vec3(d->o).sub(vec3(0, 0, d->eyeheight / 2)), vec3(0, 0, 1), 5, 1, effect::DEATH, 1);
					}
				}
		...
		//recoil
		vel.add(vec3(yaw * RAD, pitch * RAD).mul( -1 * (attack->recoil + (ammo ? ammo->recoil : 0))));
...
const char *rpgchar::getmdl() const
{
...
void rpgchar::doai(equipment *eleft, equipment *eright, equipment *quiver)
{
	...
	{
		vec3 dir = vec3(lastknown).sub(feetpos());
	...
	else
	{
		if(route.length() > 1)
		{
			vec3 dir = vec3(ai::waypoints[route[route.length() - 2]].o).sub(feetpos());
			dir.mul(curtime / 200.f / dir.magnitude());
			dir.add(vec3(yaw * RAD, pitch * RAD).mul(1 - (curtime / 200.0f)));
	...
	{
		vec3 dir = vec3(ai::waypoints[route.last()].o).sub(feetpos());
...
void rpgchar::render()
{
	...
	loopv(equipped)
	{
		...
		emitter[0] = emitter[1] = vec3(-1, -1, -1);
		emitter += 2;
	}
}
...
void rpgchar::hit(rpgent *attacker, use_weapon *weapon, use_weapon *ammo, float mul, int flags, vec3 dir)
{
	hit_friendly = 1;
	hit_total = 0;
	...
	vec3 kickback = dir.mul(weapon->kickback + (ammo ? ammo->kickback : 0));
...
int rpgchar::drop(item *it, int q, bool spawn)
{
	...
	if(rem && spawn)
	{
		...
		drop->quantity = rem;
		drop->o = vec3(yaw * RAD, pitch * RAD).mul(radius * 2).add(o);
		entinmap(drop);
	}
...
bool rpgchar::cansee(rpgent *d)
{
	...
	vec3 dir = vec3(d->o).sub(o).normalize();
	vec3 view = vec3(yaw * RAD, pitch * RAD);
