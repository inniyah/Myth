...
// 	)
	...
	struct collision
	{
		...
		collision(rpgent *de, rpgent *oe, const vec3 &ddir) : d(de), o(oe), dir(ddir) {}
		collision(rpgent *de, rpgent *oe) : d(de), o(oe) {}

		static void add(physent *d, physent *o, const vec3 &dir);
		static void add(physent *d, physent *o);
	};
	vector<collision> colcache, mergecache;

	void collision::add(physent *d, physent *o, const vec3 &dir)
	{
		loopv(colcache)
		{
...
//
// 				vel1.add(vec3(first->vel).add(first->falling).mul(.2));;
// 				vel2.add(vec3(second->vel).add(second->falling).mul(.2));
...
// 		curmap->getsignal("update");
		...
		switch(material)
		{
			case MAT_LAVA:
				...
				loopi(60)
				{
					...
					regular_particle_flame(PART_SMOKE, vec3(o.x, o.y, o.z + 8.0f), 6, 2, 0x303020, 1, 4.0f, 100.0f, 2000.0f, -20);
				}
				break;
			case MAT_WATER:
				if (waterlevel==0) break;
	...
	bool intersect(rpgent *d, const vec3 &from, const vec3 &to, float &dist)   // if lineseg hits entity bounding box
	{
		vec3 bottom(d->feetpos()), top(d->o);
	...
	rpgent *intersectclosest(const vec3 &from, const vec3 &to, rpgent *at, float &bestdist, float maxdist)
	{
		rpgent *best = NULL;
		bestdist = maxdist;
		float dist;
	...
	const char *gameconfig() { return "config/rpg/game.cfg"; }
	const char *savedconfig() { return "config/rpg/saved.cfg"; }
	const char *restoreconfig() { return "config/rpg/restore.cfg"; }
	const char *defaultconfig() { return "config/rpg/default.cfg"; }
	const char *autoexec() { return "config/rpg/autoexec.cfg"; }
	const char *savedservers() { return "config/rpg/servers.cfg"; }
}
