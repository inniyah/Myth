...
bool areaeffect::update()
{
	...
	loopvj(game::curmap->objs)
	{
		...
		const vec3 min = vec3(victim->o.x - victim->radius, victim->o.y - victim->radius, victim->o.z - victim->eyeheight);
		const vec3 max = vec3(victim->o.x + victim->radius, victim->o.y + victim->radius, victim->o.z + victim->aboveeye);
...
void areaeffect::render()
{
	...
	{
		vec3 col = vec3(fx->lightcol).mul(256);
