...
namespace entities
{
	...
	void initareatriggers()
	{
		...
		loopv(ents)
		{
			...
			vec3 radius = vec3(e.attr[0], e.attr[1], e.attr[2]).max(1);
			at.bottom = vec3(e.o).sub(radius);
			at.top = vec3(e.o).add(radius);
		}
	}
		...
		loopv(intents)
		{
			...
			switch(e.type)
			{
				case JUMPPAD:
					...
					d->vel.add(vec3(e.attr[2], e.attr[1], e.attr[0]).mul(10));
...
#undef DUMMIES
	...
	void spawn(const extentity &e, const char *id, int type, int qty)
	{
		...
		{
			ent->o.add(vec3(rnd(360) * RAD, 0).mul(rnd(e.attr[1])));
			entinmap(ent);
		}
		else
		{
	...
	void entradius(extentity &e, bool color)
	{
		switch(e.type)
		{
			...
			case JUMPPAD:
			{
				vec3 dir(e.attr[2], e.attr[1], e.attr[0]); dir.normalize();
			...
			case CAMERA:
			{
				vec3 dir(e.attr[1] * RAD, e.attr[2] * RAD);
				renderentarrow(e, dir, 12);
	...
	const char *entname(int i)
	{
		static const char *entnames[] =
		{
			...
			"teledest", "jumppad", "checkpoint", "spawn", "location", "reserved", "blip", "camera", "platformroute",
			"critter", "item", "obstacle", "container", "platform", "trigger", "areatrigger"
		};
