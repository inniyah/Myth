...
namespace game
{
	...
	int selectcrosshair(vec3 &color)
	{
	...
	void drawminimap(rpgent *d, float x, float y, float dx, float dy)
	{
		...
		if(minimapup) hudmatrix.rotate((d->yaw + 180) * RAD, vec3(0, 0, -1));
		flushhudmatrix();
		...
		if(curmap->flags & mapinfo::F_NOMINIMAP)
		{
			...
			loopi(4)
			{
				vec3 dir((d->yaw) * RAD + M_PI * i / 2.0f, 0);
		...
		{
			vec3 pos = vec3(d->o).sub(minimapcenter).mul(minimapscale).add(.5f);
			gle::colorf(1, 1, 1);
			bindminimap();
			...
			loopv(curmap->objs)
			{
				...
				vec3 pos = vec3(m->o).sub(d->o).div(offset); pos.z = 0;
				...
				loopi(4)
				{
					vec3 dir((m->yaw) * RAD + M_PI * i / 2.0f, 0);
			...
			loopv(curmap->projs)
			{
				...
				vec3 pos = vec3(p.o).sub(d->o).div(offset); pos.z = 0;
				if(pos.magnitude() >= 1)
					continue;
	...
	void drawtext(float x, float y, float size, int colour, const char *str)
	{
		...
		vec3 col((colour >> 16) & 255, (colour >> 8) & 255, colour & 255);
