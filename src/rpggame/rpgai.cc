...
namespace ai
{
	...
	{
		rpgent *target;
		vec3 lastknown, lastvel;

		attack() : directive(0), target(NULL), lastknown(vec3(0,0,0)), lastvel(vec3(0, 0, 0)) {}
	...
	struct move : directive
	{
		...
		move() : directive(0), pos(vec3(0,0,0)) {}
		move(vec3 &o, int p) : directive(p), pos(o) {}
		~move() {}
	...
	struct follow : directive
	{
		...
		follow() : directive(0), target(NULL), lastknown(vec3(0, 0, 0)), distance(0) {}
		...
		bool update(rpgchar *d)
		{
			...
			if(!(d->aiflags & AI_MOVE))
			{
				...
				{
					d->lastknown = lastknown;
					d->dest = vec3(lastknown).add(vec3(lastknown).sub(d->o).rescale(dist));
				}
			}
	...
	struct wander : directive
	{
		...
		wander() : directive(0), centre(vec3(0,0,0)), dest(vec3(0, 0, 0)), radius(0), lastupdate(0) {}
		wander(vec3 &o, int r, int p) : directive(p), centre(o), dest(o), radius(r), lastupdate(0) {}
		~wander() {}
		...
		bool update(rpgchar *d)
		{
			...
			if(lastupdate + (radius * 50) < lastmillis)
			{
				...
				dest = vec3(rnd(360) * RAD, 0).mul(rnd(radius + 1)).add(centre);
			}
	...
	struct watch : directive
	{
		...
		watch() : directive(0), target(NULL), lastknown(vec3(0, 0, 0)) {}
		~watch() {}
