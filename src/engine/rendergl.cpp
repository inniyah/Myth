...
void setcammatrix()
{
    ...
    if(!drawtex)
    {
        ...
        vec3 dir1, dir2;
        UI::getcursorpos(x, y);
        dir1 = invcamprojmatrix.perspectivetransform(vec3(x*2-1, 1-2*y, 2-1));
        dir2 = invcamprojmatrix.perspectivetransform(vec3(x*2-1, 1-2*y, -1));

        dir1.sub(dir2).normalize();
        if(raycubepos(camera1->o, dir1, worldpos, 0, RAY_CLIPMAT|RAY_SKIPFIRST) == -1)
            worldpos = vec3(camdir).mul(2 * worldsize).add(camera1->o);
    }
}
...
void drawcrosshair(int w, int h)
{
    ...
    else
    {
        ...
        if(index < 0) return;
        if(!crosshaircol) color = vec3(1, 1, 1);
