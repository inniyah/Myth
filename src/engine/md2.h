...
struct md2 : vertloader<md2>
{
        ...
        bool load(const char *filename, float smooth)
        {
            ...
            loopi(header.numframes)
            {
                ...
                loopj(m.numverts)
                {
                    ...
                    curvert->pos = vec3(v.vertex[0]*frame.scale[0]+frame.translate[0],
                    ...
                    curvert->norm = vec3(norm[0], -norm[1], norm[2]);
                    ++curvert;
                }
    ...
    bool loaddefaultparts()
    {
        ...
        defformatstring(name1, "media/models/%s/tris.md2", name);
        mdl.meshes = sharemeshes(path(name1));
        if(!mdl.meshes)
        {
            defformatstring(name2, "media/models/%s/tris.md2", pname);    // try md2 in parent folder (vert sharing)
        ...
        defformatstring(name3, "media/models/%s/md2.cfg", name);
        if(!execfile(name3, false))
        {
            formatstring(name3, "media/models/%s/md2.cfg", pname);
