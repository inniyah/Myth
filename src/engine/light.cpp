...
struct dynlightprops
{
    int lastupdate, num;
    int radius, red, green, blue;

    dynlightprops() : lastupdate(0), num(0), radius(0), red(0), green(0), blue(0) {}
    ~dynlightprops() {}
};
...
void lightreaching(const vec3 &target, vec3 &color, vec3 &dir, bool fast, extentity *t, float minambient)
{
    ...
    loopv(lights)
    {
        ...
        vec3 lightcol = vec3(red, green, blue).mul(1.0f/255).max(0);
