...
void rendereditcursor()
{
    ...
    vec3 dir = vec3(worldpos).sub(camera1->o).normalize();
    ...
    else
    {
        ...
        if((havesel || dragging) && !passthroughsel && !hmapedit)     // now try selecting the selection
            if(rayboxintersect(vec3(sel.o), vec3(sel.s).mul(sel.grid), camera1->o, dir, sdist, orient))
        ...
        {
            vec3 w = vec3(dir).mul(wdist+0.05f).add(camera1->o);
            if(!insideworld(w))
            {
                loopi(3) wdist = min(wdist, ((dir[i] > 0 ? worldsize : 0) - camera1->o[i]) / dir[i]);
                w = vec3(dir).mul(wdist-0.05f).add(camera1->o);
                if(!insideworld(w))
                {
                    wdist = 0;
            ...
            if(sdist == 0 || sdist > wdist) rayboxintersect(vec3(lu), vec3(gridsize), camera1->o, dir, t=0, orient); // just getting orient
            cur = lu;
    ...
    if(u->numents)
    {
        ...
        loopi(u->numents)
        {
            *(ushort *)buf.pad(2) = lilswap(ushort(ue[i].i));
            ...
            lilswap(&e.o.x, 3);
            lilswap(e.attr.getbuf(), e.attr.length());
            lilswap(&e.attr.len);
        }
    }
    else
