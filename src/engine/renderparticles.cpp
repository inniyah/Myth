 ...
 */
void regularshape(int type, int radius, int color, int dir, int num, int fade, const vec3 &p, float size, float vel, int gravity)
{
    ...
    loopi(num)
    {
        ...
        else
        {
            ...
            if(fade < 0 && parts[type]->type & PT_COLLIDE)
                n->val = from.z - raycube(from, vec3(0, 0, -1), COLLIDERADIUS, RAY_CLIPMAT) + COLLIDEERROR;
        }
    }
}
...
void regularlensflare(vec3 o, uchar r, uchar g, uchar b, bool sun, bool sparkle, int sizemod)
{
...
static void makeparticles(entity &e)
{
    ...
    switch(attr1)
    {
        case 0: //fire and smoke -  <radius> <height> <rgb> - 0 values default to compat for old maps
        {
            ...
            regularflame(PART_SMOKE, vec3(e.o.x, e.o.y, e.o.z + 4.0f*min(radius, height)), radius, height, 0x303020, 1, attr5 ? .02f * abs(attr5) : 4.0f, attr6 ? 0.01f * abs(attr6) : 100.0f, attr7 ? abs(attr7) + 1400: 2000.0f, attr8 ? attr8 - 5 : -20.0f);
            break;
        }
        ...
        case 5: //fire ball - <size> <rgb>
           newparticle(e.o, vec3(0, 0, 1), 1, PART_EXPLOSION, attr3 & 0xFFFFFF, 4.0f)->val = 1+attr2;
            break;
        case 6: //meter, metervs - <percent> <rgb> <rgb2>
        case 7:
        {
            particle *p = newparticle(e.o, vec3(0, 0, 1), 1, attr1==6 ? PART_METER : PART_METER_VS, attr3 & 0xFFFFFF, 2.0f);
