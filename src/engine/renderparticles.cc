...
static void makeparticles(entity &e)
{
    switch(e.attr[0])
    {
        ...
        case 3: //fire ball - <size> <rgb>
            newparticle(e.o, vec3(0, 0, 1), 1, PART_EXPLOSION, colorfromattr(e.attr[2]), 4.0f)->val = 1+e.attr[1];
            break;
        ...
        case 6:
        {
            particle *p = newparticle(e.o, vec3(0, 0, 1), 1, e.attr[0]==5 ? PART_METER : PART_METER_VS, colorfromattr(e.attr[2]), 2.0f);
...
bool printparticles(extentity &e, char *buf, int len)
{
    switch(e.attr[0])
    {
        case 0: case 4: case 7: case 8: case 9: case 10: case 11: case 12: case 13:
            nformatstring(buf, len, "%s %d %d %d 0x%.3hX %d", entities::entname(e.type), e.attr[0], e.attr[1], e.attr[2], e.attr[3], e.attr[4]);
            return true;
        case 3:
            nformatstring(buf, len, "%s %d %d 0x%.3hX %d %d", entities::entname(e.type), e.attr[0], e.attr[1], e.attr[2], e.attr[3], e.attr[4]);
            return true;
        case 5: case 6:
            nformatstring(buf, len, "%s %d %d 0x%.3hX 0x%.3hX %d", entities::entname(e.type), e.attr[0], e.attr[1], e.attr[2], e.attr[3], e.attr[4]);
            return true;
    }
    return false;
}
