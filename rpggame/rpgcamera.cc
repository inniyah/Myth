...
namespace camera
{
	...
	struct move : action
	{
		...
		bool update()
		{
			camera.o.add(vec3(dx, dy, dz).mul(interp(startmillis, duration, true)));
		...
		void debug(int &hoffset, bool type)
		{
			...
			draw_textf("Delta: vec3(%f, %f, %f)", 100, 0 + hoffset, dx, dy, dz);
			hoffset += 64;
	...
	struct focus : action
	{
		...
		bool update()
		{
			vec3 ideal = vec3(ent->vel).add(ent->falling).mul(lead).add(ent->o).sub(camera.o).normalize();
			vec3 current = vec3(camera.yaw * RAD, camera.pitch * RAD);
			float mul = 1;
	...
	struct follow : action
	{
		...
		bool update()
		{
			vec3 pos = vec3(ent->yaw * RAD, ent->pitch * RAD).mul(-tail).add(ent->o);
			pos.z += height;
			float mul = 1;
	...
	struct scale : container
	{
		...
		{
			float mul = interp(startmillis, duration, false);
			vec3 d = vec3(dx, dy, 1).mul(mul).add(1 - mul);
	...
	struct rotate : container
	{
		...
		{
			pushhudmatrix();
			hudmatrix.rotate(RAD * angle * interp(startmillis, duration, false), vec3(0, 0, -1));
			flushhudmatrix();
			container::render(w, h);
			pophudmatrix();
		}
	...
	ICOMMAND(cs_action_movecamera, "iie", (int *tag, int *d, uint *s),
		...
		if(cam)
		{
			vec3 delta = vec3(cam->o).sub(camera.o);
	...
	ICOMMAND(cs_action_moveent, "sie", (const char *r, int *d, uint *s),
		...
		{
			vec3 delta = vec3(ent->getent(entidx)->o).sub(camera.o);
			actionvec->add(new move(delta.x, delta.y, delta.z, *d, s));
		}
		else
		{
	...
	ICOMMAND(cs_action_viewcamera, "iie", (int *tag, int *d, uint *s),
		...
		if(cam)
		{
			vec3 delta = vec3(cam->attr[1], cam->attr[2], cam->attr[3]).sub(vec3(camera.yaw, camera.pitch, camera.roll));
	...
	ICOMMAND(cs_action_viewent, "sie", (const char *r, int *d, uint *s),
		...
		if(ent && ent->getent(entidx))
		{
			vec3 delta = vec3(ent->getent(entidx)->yaw, ent->getent(entidx)->pitch, ent->getent(entidx)->roll).sub(vec3(camera.yaw, camera.pitch, camera.roll));
			actionvec->add(new view(delta.x, delta.y, delta.z, *d, s));
		}
		else
		{
