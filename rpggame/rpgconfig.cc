...
namespace game
{
	#define CHECK(x, c, vec3, select) \
		...
		ICOMMAND(r_select_ ## c, "se", (const char *ref, uint *contents), \
			...
			if(!loading ##x) \
			{ \
				if((loading ## x = vec3.access(ref)) && DEBUG_VCONF) \
		...
		ICOMMAND(r_num_ ## c, "", (), \
			if(DEBUG_VCONF) \
				DEBUGF("r_num_" #c " requested, returning %i", vec3.length()); \
			intret(vec3.length()); \
		) \
	...
	#define VECNF(name, var, l1, l2, l3, h1, h2, h3, body) \
		START(name, "fffN$", (float *x, float *y, float *z, int *numargs, ident *self), \
			...
			const vec3 res(*x, *y, *z); \
			const vec3 vmin(h1, h2, h3); \
			const vec3 vmax(l1, l2, l3); \
			e->var = vec3(res).min(vmin).max(vmax); \
			body; \
			if(e->var != res) \
				WARNINGF("value provided for " DEBUG_STR "->" #var " exceeded limits", DEBUG_IND); \
