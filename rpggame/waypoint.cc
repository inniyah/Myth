...
namespace ai
{
	...
	waypoint *closestwaypoint(const vec3 &o)
	{
	...
	void dropwaypoint(const vec3 &o)
	{
		...
		bool wpair = !isliquid(wmat) && raycube(wp.o, vec3(0, 0, -waypointstairheight), 0, RAY_POLY|RAY_CLIPMAT) >= 1;
		...
		for(int i = 0; i < waypoints.length() - 1; i++)
		{
			...
			bool owair = !isliquid(omat) && raycube(ow.o, vec3(0, 0, -waypointstairheight), 0, RAY_POLY|RAY_CLIPMAT) >= 1;
			...
			else
			{
				...
				if(!wpair && !owair && wp.o.z != ow.o.z)
				{
					...
					{
						from = wp.o;
						raycubepos(ow.o, vec3(0, 0, 1), ray, wp.o.z - ow.o.z, RAY_POLY|RAY_CLIPMAT);
						ray.sub(from);
					}
					else
					{
						raycubepos(wp.o, vec3(0, 0, 1), from, ow.o.z - wp.o.z, RAY_POLY|RAY_CLIPMAT);
						ray = vec3(ow.o).sub(from);
					}
				}
				else
				{
					from = wp.o;
					ray = vec3(ow.o).sub(wp.o);
				}
	...
	void renderwaypoints()
	{
		loopv(waypoints)
		{
			...
			particle_textcopy(vec3(0, 0, 6).add(w.o), ds, PART_TEXT, 1, 0xFF00FF, 3);
		...
		for(int i = 1; i < testroute.length(); i++)
		{
			...
			particle_flare(vec3(0, 0, 1).add(prev.o), vec3(0, 0, 1).add(cur.o), 0, PART_STREAK, 0xFF0000, .5);
		}
		loopv(game::curmap->objs)
		{
			...
			for(int j = 1; j < ent->route.length(); j++)
			{
				...
				particle_flare(vec3(0, 0, i + 1).add(prev.o), vec3(0, 0, i + 1).add(cur.o), 0, PART_STREAK, 0x00FF00, .5);
			}
			for(int j = max(0, ent->route.length() - 2); j < ent->route.length(); j++)
			{
				vec3 from = ent->abovehead();
				vec3 dest = vec3(0, 0, i+1).add(waypoints[ent->route[j]].o);
