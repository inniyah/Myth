...
namespace ai
{
	...
	extern waypoint *closestwaypoint(const vec3 &o);
...
// 	extern vector<equipment> hotkeys;
	...
	extern bool cansave();
	extern bool intersect(rpgent *d, const vec3 &from, const vec3 &to, float &dist);
	extern rpgent *intersectclosest(const vec3 &from, const vec3 &to, rpgent *at, float &bestdist, float maxdist = 1e16);
...
struct effect
{
	...
	void drawsphere(const vec3 &o, float radius, float size = 1, int type = PROJ, int elapse = 17);
	void drawsplash(const vec3 &o, vec3 dir, float radius, float size = 1, int type = PROJ, int elapse = 17);
	bool drawline(vec3 &from, const vec3 &to, float size = 1, int type = PROJ, int elapse = 17);
	void drawwield(vec3 &from, const vec3 &to, float size = 1, int type = PROJ, int elapse = 17);
	void drawaura(physent *d, float size = 1, int type = PROJ, int elapse = 17);
	void drawcircle(const vec3 &o, vec3 dir, const vec3 &axis, int angle, float radius, float size = 1, int type = PROJ, int elapse = 17);
	void drawcircle(physent *d, use_weapon *wep, float size = 1, int type = PROJ, int elapse = 17);
	void drawcone(const vec3 &o, vec3 dir, const vec3 &axis, int angle, float radius, float size, int type = PROJ, int elapse = 17);
...
struct rpgent : dynent
{
	...
	virtual vec3 blipcol() { return vec3(1, 1, 1);}
	...
	virtual void hit(rpgent *attacker, use_weapon *weapon, use_weapon *ammo, float mul, int flags, vec3 dir) = 0;
...
struct rpgitem : rpgent, item
{
	...
	vec3 blipcol() { return vec3(0, .75, 1);}
	void hit(rpgent *attacker, use_weapon *weapon, use_weapon *ammo, float mul, int flags, vec3 dir);
...
struct rpgobstacle : rpgent
{
	...
	vec3 blipcol() { return vec3(1, 1, 1);}
	const char *getmdl() const;
...
struct rpgchar : rpgent
{
	...
	rpgchar() : name(NULL), mdl(newstring(DEFAULTMODEL)), portrait(NULL), colour(1, 1, 1), script(DEFAULTCHARSCR), faction(DEFAULTFACTION), merchant(NULL), base(stats(this)), scale(1), lastaction(0), lastai(0), lastpain(0), charge(0), primary(false), secondary(false), lastprimary(false), lastsecondary(false), attack(NULL), aiflags(0), target(NULL), forceanim(0)
	{
		...
		mana = base.getmaxmp();
		loopi(SLOT_NUM * 2) emitters[i] = vec3(-1, -1, -1);
	}
	~rpgchar()
	{
		delete[] name;
...
struct blip
{
	...
	blip(vec3 _o, vec3 _c = vec3(1,1,1), int _s = 8) : o(_o), col(_c), size(_s) {}
	~blip() {}
};
...
struct waypoint
{
	...
	waypoint() : parent(NULL), o(vec3(0, 0, 0)), score(-1) {}
	waypoint(const vec3 &o) : parent(NULL), o(o), score(-1) {}
};
