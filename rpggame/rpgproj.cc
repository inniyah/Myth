...
void projectile::init(rpgchar *d, equipment *w, equipment *a, int fuzzy, float mult, float speed)
{
	...
	dir = vec3(RAD * d->yaw, RAD * d->pitch);
	if(fuzzy > 0)
	{
		vec3 axis(rnd(360) * RAD, rnd(360) * RAD);
		dir.rotate(fuzzy * RAD / 2.f, axis);
	}
	o = d->o;

	//we now need to move the projectile outside the player's bounding box
	vec3 test = vec3(dir).mul(max(d->eyeheight, d->radius) * 4); test.add(o);
		...
		switch(amm ? amm->target : wep->target)
		{
			...
			case T_SMULTI:
				loopv(game::curmap->objs)
				{
					...
					{
						vec3 kick = vec3(d->o).sub(o).normalize();
...
{
	vec3 pos;
	float geomdist = raycubepos(o, vec3(dir).normalize(), pos, distance, RAY_CLIPMAT|RAY_ALPHAPOLY);
	...
	else
	{
		...
		if(vic && entdist <= 1)
		{
			...
			geomdist *= entdist;
			normal = vec3(pos).sub(vic->o).normalize();
		}
...
void projectile::render()
{
	if(projfx)
	{
		...
		else
		{
			...
			{
				vec3 col = vec3(projfx->lightcol).mul(256);
