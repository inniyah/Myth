...
void rpgplatform::update()
{
	...
	while(flags && F_ACTIVE && target >= 0 && delta > 0)
	{
		...
		{
			delta -= dist;
			vec3 move = vec3(entities::ents[target]->o).sub(o).normalize().mul(dist);
...
void rpgplatform::render()
{
	...
	rendermodel(getmdl(), ANIM_MAPMODEL|ANIM_LOOP, vec3(o).sub(vec3(0, 0, eyeheight)), yaw, pitch, roll, MDL_CULL_DIST|MDL_CULL_OCCLUDED, NULL, NULL, 0, 0, getscale(), col);
}

void rpgplatform::hit(rpgent *attacker, use_weapon *weapon, use_weapon *ammo, float mul, int flags, vec3 dir)
{
