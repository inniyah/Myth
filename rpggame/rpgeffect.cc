...
inline const vec3 parsevec(const char *str)
{
...
void effect::drawsphere(const vec3 &o, float radius, float size, int type, int elapse)
{
	if(size <= 0) return;
	...
	switch(particle)
	{
		...
		case PART_LIGHTNING:
			...
			loopi(num)
			{
				vec3 offset = vec3(rnd(360) * RAD, rnd(360) * RAD).mul(radius);
				vec3 offset2 = vec3(rnd(360) * RAD, rnd(360) * RAD).mul(radius);
				particle_flare(vec3(o).sub(offset), vec3(o).add(offset2), fade, particle, colour, size);
			}
			break;
		default:
			loopi(num)
...
void effect::drawsplash(const vec3 &o, vec3 dir, float radius, float size, int type, int elapse)
{
	if(size <= 0) return;
	...
	switch(particle)
	{
		...
		case PART_LIGHTNING:
			...
			{
				vec3 offset = vec3(dir).mul(-2 * size).add(o);
			...
			else
			{
				...
				loopi(num)
				{
					vec3 offset = vec3(rnd(360) * RAD, (90 - rnd(180)) * RAD).mul(radius);
					vec3 offset2 = vec3(rnd(360) * RAD, rnd(360) * RAD).mul(radius);
					particle_flare(vec3(o).sub(offset), vec3(o).add(offset2), fade, particle, colour, size);
				}
			}
...
ICOMMAND(r_effect_emitsplash, "ssffi", (const char *eff, const char *ref, float *radius, float *size, int *style),
	...
	e->drawsplash(d->o, vec3(0, 0, 1), max(1.0f, *radius), *size, clamp(*style, (int)effect::PROJ, (int)effect::DEATH_PROLONG), curtime);
)
...
ICOMMAND(r_effect_emitsphlash_pos, "ssffi", (const char *eff, const char *pos, float *radius, float *size, int *style),
	...
	e->drawsplash(parsevec(pos), vec3(0, 0, 1), max(1.0f, *radius), *size, clamp(*style, (int)effect::PROJ, (int)effect::DEATH_PROLONG), curtime);
)

VARP(linemaxsteps, 8, 32, 1024);
VARP(linemininterval, 1, 8, 32);

bool effect::drawline(vec3 &from, const vec3 &to, float size, int type, int elapse)
{
	...
	if(!num) return false;
	vec3 delta = vec3(to).sub(from).div(num);
	...
	loopi(num)
	{
		switch(particle)
		{
			...
			case PART_LIGHTNING:
			{
				...
				vec3 start = vec3(rnd(360) * RAD, rnd(360) * RAD).mul(4 * size).add(from);
				vec3 end = vec3(delta).mul(1.5).add(start);
...
void effect::drawwield(vec3 &from, const vec3 &to, float size, int type, int elapse)
{
	if(size <= 0) return;
	...
	vec3 delta = vec3(to).sub(from);

	loopi(num)
	{
		vec3 pt = vec3(delta).mul((float) rnd(0x10000) / 0xFFFF).add(from);
		switch(particle)
		{
			case PART_EXPLOSION:
...
void effect::drawaura(physent *d, float size, int type, int elapse)
{
	...
	loopi(num)
	{
		vec3 pos = vec3(rnd(360) * RAD, 0).mul(d->radius + size).add(d->feetpos());
		switch(particle)
		{
			case PART_EXPLOSION:
			...
			case PART_LIGHTNING:
				if(!curtime) return;
				particle_flare(pos, vec3(0, 0, d->eyeheight + d->aboveeye).add(pos), fade, particle, colour, size);
				break;
			default:
				if(gravity >= 0)
					pos.add(vec3(0, 0, d->eyeheight + d->aboveeye));
...
void effect::drawcircle(const vec3 &o, vec3 dir, const vec3 &axis, int angle, float radius, float size, int type, int elapse)
{
	if(size <= 0) return;
...
void effect::drawcircle(physent *d, use_weapon *wep, float size, int type, int elapse)
{
	...
	if(wep->target == T_HORIZ)
		axis = vec3(d->yaw * RAD, (d->pitch + 90) * RAD);
	else
		axis = vec3((d->yaw + 90) * RAD, 0);

	vec3 dir = vec3(d->yaw * RAD, d->pitch * RAD).rotate(-wep->angle / 2.f * RAD, axis);
	vec3 o = vec3(d->o).sub(vec3(0, 0, d->eyeheight / 2));
	drawcircle(o, dir, axis, wep->angle, d->radius + wep->range, size, type, elapse);
}

void effect::drawcone(const vec3 &o, vec3 dir, const vec3 &axis, int angle, float radius, float size, int type, int elapse)
{
	if(size <= 0) return;
	...
	loopi(num)
	{
		vec3 ray = vec3(dir).rotate(rnd(angle + 1) * RAD, axis).rotate(rnd(360), dir).mul(radius);
		switch(particle)
		{
			case PART_EXPLOSION:
				ray.mul(rnd(101)/100.f).add(o);
...
{
	vec3 dir = vec3(d->yaw * RAD, d->pitch * RAD);
	vec3 axis = vec3(d->yaw * RAD, (d->pitch + 90) * RAD);
